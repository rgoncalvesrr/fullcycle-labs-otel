...
func main() {
	...
	shutdown, err := otel_provider.InitProvider(configs.Cfg.OrchestratorApiServiceName, configs.Cfg.OpenTelemetryCollectorExporterEndpoint)
	if err != nil {
		log.Fatal(err)
	}
	...
	w := infra.NewWebServer(configs.Cfg.OrchestratorApiPort)
	w.RegisterHandler("GET /{cep}", adapter.NewWeatherHandler(tracer).Get)

	go func() {
		log.Printf("Iniciando servidor na porta %s", configs.Cfg.OrchestratorApiPort)
		w.Start()
	}()
